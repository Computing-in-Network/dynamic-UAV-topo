...
- 前端支持语义网络退化态势展示：
  - 新增“语义退化态势”面板，显示健康链路/弱链路/物理遮挡/语义退化统计
  - 在语义模式（`/swarm/state_semantic`）下，链路颜色可区分物理遮挡与语义退化
...
- 信息来源：可视化服务返回 `semantic_mode` 与 `swarm_topic`，链路基础字段使用 `weight/is_occluded`。
- 区分规则（当前版本）：
  - `is_occluded=true` 归类为物理遮挡；
  - 语义模式下 `is_occluded=false` 且低权重链路归类为语义退化（含高退化）。
- 当前局限：由于消息中尚无“退化来源”结构化字段，语义退化分类为前端可视化规则，不等价于后端全部内部状态。
