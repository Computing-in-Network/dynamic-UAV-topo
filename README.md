...
- 动态火区演化：火区半径支持扩张与收缩；当 UAV 靠近热点时，火区响应速度加快（观测耦合）。
- 验证圆轮廓弱化：默认不突出圆形轮廓，降低“验证图形”干扰。
- 邻近热点聚类展示：相邻火点会自动形成小尺度聚类区域，避免单点割裂感。
- 自主探测增强：当热点数量少于 UAV 数量时，规划器允许多 UAV 跟随同一热点，提高跟踪可见性。
- 火情视觉增强：热点点位改为火焰 icon，聚类区域由平滑多边形表示（非圆形近似）。
- 火点贴地与多点簇：火焰点默认贴地渲染，并按热点强度生成地面子火点簇，提升“自主发现/跟踪”可见性。
- UAV 跟踪平滑：管理节点按“平滑速度跟踪 + 近目标减速”更新位置，减少硬拐和抖动。
...
首发版本发布说明见：`docs/release-v1.0.0.md`。
