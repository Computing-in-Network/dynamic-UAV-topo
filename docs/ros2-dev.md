...
- 右侧“模式”支持 `实时/回放` 切换
- 回放模式支持 `播放/暂停/重置/拖动时间轴`
- 回放速率支持 `0.5x / 1x / 2x`
- 切回实时模式后恢复最新帧刷新

图层与视图控制（P2）：

- 图层开关：`链路`、`火点`、`火区`、`任务`（互不影响，可独立开关）。
- 视图控制：
  - `重置视角`：回到默认观察位置。
  - `聚焦机群`：相机移动到当前 UAV 群中心区域。
  - `聚焦火场`：相机移动到当前火点/火区中心区域；若无火情则回退到机群视角。

任务解释面板（P2）：

- 基础交互：点击 UAV 表行或任务表行，面板切换到对应 UAV 的任务解释。
- 展示内容：`uav_id`、目标点坐标、优先级、`reason` 原文。
- 关键因子：前端对 `reason` 文本做关键词摘要，当前支持距离/保持稳定/切换惩罚/覆盖抑制。
- 来源与局限：
  - 来源：`MissionPlan.targets[].reason`（文本）；
  - 局限：非结构化文本场景下，摘要因子仅用于可读性增强，不等价于后端完整评分细节。
...
- 链路可视区分：
  ...
  - 语义高退化：红色虚线
  - 健康链路：绿色实线
- 面板附带 `semantic_mode` 与 `swarm_topic`，用于区分普通拓扑与语义网络输出。
...
- HTTP API：`/api/swarm_state` 返回单帧 JSON 快照。
- 必需字段：`timestamp`（毫秒）；其余字段可为空或缺省。
- 推荐字段：`uavs`、`links`、`fire_hotspots`、`fire_regions`、`mission_targets`。
- 语义态势字段（可选）：`semantic_mode`（bool）、`swarm_topic`（string）。
- 历史帧来源：前端缓存，不依赖后端历史接口（当前默认窗口 `120` 帧，轮询间隔 `500ms`）。
- 同步语义：回放时按同一缓存帧渲染火点、火区、任务目标，确保三类信息时间一致。
