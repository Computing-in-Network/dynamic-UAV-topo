...
ros2 run swarm_uav_manager swarm_uav_manager_node --ros-args \
  ...
  > /tmp/swarm_manager_profile.log 2>&1 &
MANAGER_PID=$!
...
ros2 run swarm_topology_analyzer swarm_topology_analyzer_node --ros-args \
  ...
  > /tmp/swarm_topology_profile.log 2>&1 &
TOPO_PID=$!

sleep "${DURATION_SECONDS}"

profile_line="$(grep 'profile frames=' /tmp/swarm_topology_profile.log | tail -n 1 || true)"
if [[ -z "${profile_line}" ]]; then
  echo "[profile_topology_20] FAIL: 未采集到 profile 日志"
  echo "--- topology log ---"
  cat /tmp/swarm_topology_profile.log || true
  exit 1
fi

echo "[profile_topology_20] ${profile_line}"
if [[ "${profile_line}" == *"status=PASS"* ]]; then
  echo "[profile_topology_20] PASS: 拓扑单帧耗时满足目标(<${TARGET_MS}ms)"
else
  echo "[profile_topology_20] WARN: 拓扑单帧耗时未达到目标(<${TARGET_MS}ms)"
  exit 2
fi
